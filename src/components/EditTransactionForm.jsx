...
export default function EditTransactionForm({ transaction, onSave, onCancel }) {
  ...
  return <div className="bg-white dark:bg-gray-900 rounded-2xl p-4 shadow-sm border border-gray-200 dark:border-gray-800 space-y-4">
    ...
    <div className="bg-slate-50 rounded-xl px-3 py-2 text-xs text-slate-500 dark:text-gray-400 flex items-center gap-2">
    ...
    <form onSubmit={handleSubmit} className="space-y-3">
      <input type="date" value={form.date || ''} onChange={(e) => change('date', e.target.value)} className="w-full border border-gray-300 dark:border-gray-700 rounded-xl px-3 py-2 text-sm" required />
      <div className="grid grid-cols-2 gap-2"><Select value={form.type || 'Expense'} onChange={change.bind(null, 'type')}><option>Income</option><option>Expense</option></Select><input value={form.currency || ''} readOnly className="border border-gray-200 bg-gray-50 rounded-xl px-3 py-2 text-sm text-gray-500 dark:text-gray-400" /></div>
      <Select value={form.account || 'Cash'} onChange={change.bind(null, 'account')}><option>Cash</option><option>Bank</option></Select>
      <input value={form.sourceCategory || ''} onChange={(e) => change('sourceCategory', e.target.value)} placeholder="Category / Source" className="w-full border border-gray-300 dark:border-gray-700 rounded-xl px-3 py-2 text-sm" required />
      <input value={form.whereVendor || ''} onChange={(e) => change('whereVendor', e.target.value)} placeholder="Where / Vendor" className="w-full border border-gray-300 dark:border-gray-700 rounded-xl px-3 py-2 text-sm" />
      <input value={form.description || ''} onChange={(e) => change('description', e.target.value)} placeholder="Description" className="w-full border border-gray-300 dark:border-gray-700 rounded-xl px-3 py-2 text-sm" />
      <input type="number" step="any" value={form.amount || ''} onChange={(e) => change('amount', e.target.value)} placeholder="Amount" className="w-full border border-gray-300 dark:border-gray-700 rounded-xl px-3 py-2 text-sm" required />
      <textarea value={form.note || ''} onChange={(e) => change('note', e.target.value)} placeholder="Note" rows="2" className="w-full border border-gray-300 dark:border-gray-700 rounded-xl p-3 text-sm resize-none"></textarea>
