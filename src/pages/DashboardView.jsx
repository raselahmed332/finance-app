...
export default function DashboardView({ wallets, walletSummaries, setActiveTab, can }) {
  return (
    <div className="space-y-4">
      ...
      {wallets.length > 0 && (
        <div className="pt-2">
          ...
          <div className="grid grid-cols-5 gap-1.5">
            ...
            {can('TRANSFER_MONEY') && <button onClick={() => setActiveTab('transfer')} className="bg-purple-600 hover:bg-purple-700 text-white rounded-xl p-2 flex flex-col items-center justify-center gap-1 shadow-sm transition-transform active:scale-95">
      ...
      {(can('ADD_USER') || can('EDIT_USER') || can('DELETE_USER') || can('MANAGE_USER_PERMISSIONS')
        || can('ADD_WALLET') || can('EDIT_WALLET') || can('MANAGE_WALLET_STATUS')
        || can('BACKUP_RESTORE') || can('MANAGE_CATEGORIES') || can('VIEW_AUDIT_LOG')) && (
        <div className="grid grid-cols-2 gap-2 pt-1">
          {(can('ADD_USER') || can('EDIT_USER') || can('DELETE_USER') || can('MANAGE_USER_PERMISSIONS')) && <button onClick={() => setActiveTab('users')} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-2.5 flex items-center justify-between text-xs font-semibold text-slate-700 dark:text-gray-200 shadow-2xs hover:bg-gray-50 dark:hover:bg-gray-800">
          ...
          {(can('ADD_WALLET') || can('EDIT_WALLET') || can('MANAGE_WALLET_STATUS')) && <button onClick={() => setActiveTab('wallets')} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-2.5 flex items-center justify-between text-xs font-semibold text-slate-700 dark:text-gray-200 shadow-2xs hover:bg-gray-50 dark:hover:bg-gray-800">
            <span><i className="fa-solid fa-vault text-emerald-600 me-2"></i>Wallet ম্যানেজমেন্ট</span>
            <i className="fa-solid fa-chevron-right text-gray-400 dark:text-gray-500 text-xs"></i>
          </button>}
          {(can('BACKUP_RESTORE') || can('MANAGE_CATEGORIES')) && <button onClick={() => setActiveTab('settings')} className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-2.5 flex items-center justify-between text-xs font-semibold text-slate-700 dark:text-gray-200 shadow-2xs hover:bg-gray-50 dark:hover:bg-gray-800">
